...
1. 所有代码模块的模块级注释、模块级文档字符串，必须使用简体中文；新增或修改代码时必须执行本规则。
2. 注释必须结合模块的真实生产业务，说明模块负责的业务对象、关键流程、边界条件或失败处理；禁止使用“工具类”“处理数据”等脱离实际业务的空泛描述。
3. 修改现有模块时，应同步校正不符合真实实现、已经过期或仅为占位说明的模块注释；禁止保留会误导维护者的描述。
4. 标准协议名、API 名、类名、函数名、字段名和必要技术术语可以保留英文，但其业务含义与作用必须用简体中文说明。
5. 注释只描述代码能够证明的行为；未实现、未测试或仅规划的能力，必须明确标注状态，不得写成已经具备。
6. 代码评审和提交前检查必须包含本规则。后续新增或修改的模块若不符合本规则，不得标记为开发完成。
