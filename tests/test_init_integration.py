...
def _run_in_subprocess(
    ...
    timeout: float = 5.0,
    signal_after_stdout: str | None = None,
) -> tuple[int, str, str]:
    """Run ``code`` in a fresh Python subprocess, optionally sending it
    ``signal_name`` after ``signal_after_ms`` ms or after stdout contains
    ``signal_after_stdout``. Returns (returncode, stdout, stderr).
    """
    ...
    out_prefix = b""
    if signal_after_stdout is not None and proc.stdout is not None:
        marker = signal_after_stdout.encode("utf-8")
        ...
        while time.monotonic() < deadline and proc.poll() is None:
            ...
            if marker in line:
                break
    elif signal_after_ms is not None:
        time.sleep(signal_after_ms / 1000.0)

    if signal_after_ms is not None and signal_after_stdout is not None:
        time.sleep(signal_after_ms / 1000.0)
    if signal_after_ms is not None or signal_after_stdout is not None:
        try:
    ...
    return (
        proc.returncode,
        (out_prefix + out).decode("utf-8", errors="replace"),
...
class TestSigintDuringPrefetch(unittest.TestCase):
    ...
    def test_sigint_during_prefetch_clean_exit(self) -> None:
        ...
        rc, out, err = _run_in_subprocess(
            code, signal_name="SIGINT", signal_after_stdout="READY"
        )
