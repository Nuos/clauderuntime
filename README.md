...
- 以 `docs/plans/active/seven-component-optimization-development-plan.md` 和 `docs/parity/clauderuntime-source-parity-action-bible-v1.0.md` 为当前开发规范。
...
- `docs/parity/diagnostics/2026-08-10-diagnostic.md`：诊断基线。
- `docs/plans/active/seven-component-optimization-development-plan.md`：阶段开发计划。
- `docs/parity/clauderuntime-source-parity-action-bible-v1.0.md`：source parity 总规范。
- `docs/progress/2026/2026-08-10-development-progress.md`：开发进度、完成度、修改模块与验证记录。
- `docs/parity/`：source map、runtime path、auxiliary map 等机器可读 parity 资产。

历史文档归档在 `docs/archive/`，该目录仅用于保留旧资料，不作为当前状态真值。
...
已提交进度见 `docs/progress/2026/2026-08-10-development-progress.md`。当前分支已推进：
...
- 不新增 `docs/` 下生成的 `.html` 文件；既有历史 HTML 仅随迁移保留。
- 不把 `docs/archive/` 作为当前事实源；迁移或归档旧资料时必须标明状态。
- 文档与 PR 说明优先使用简体中文。
