...
Claw Codex is the Python port of Claude Code. The current architecture
governance follows the Reference-7 component model used by the parity
program:
...
`Context / Memory` is not a standalone eighth component. Context shaping,
prompt construction, and compaction belong to Agent Loop; durable memory,
transcripts, and resume semantics belong to State & Persistence.

For current parity status and the 7 x 5 x 14 evidence model, see
[../parity/README.md](../parity/README.md). For active implementation
plans, see [../plans/active/](../plans/active/).

---

## Reference-7 component map

| # | Component | Python canonical paths | Primary evidence |
|---|---|---|---|
| 1 | User | `src/cli.py`, `src/entrypoints/`, `ui-tui/`, `ui-desktop/` | Cross-surface behavior and session entry contracts |
| 2 | Interfaces | `src/server/agent_server.py`, `src/entrypoints/serve_cli.py`, `ui-tui/`, `ui-desktop/` | Streaming, interrupts, permissions UI, resume surfaces |
| 3 | Agent Loop | `src/query/`, `src/coordinator/`, `src/tasks/` | Query loop, tool rounds, retry, compaction, terminal reasons |
| 4 | Permission System | `src/permissions/` | Modes, rules, hooks, workspace boundary, trust gates |
| 5 | Tools | `src/tool_system/`, `src/services/tool_execution/`, `src/services/mcp/` | Tool schema, execution, concurrency, MCP lifecycle |
| 6 | State & Persistence | `src/bootstrap/`, `src/state/`, `src/session/`, `src/memdir/` | Transcripts, memory, session recovery, resume/fork semantics |
| 7 | Execution Environment | `src/utils/subprocess_*.py`, `src/workflow/`, `src/services/remote/`, platform helpers | Sandbox, process, cwd, filesystem, worktree, remote execution |

---

## Legacy source-navigation map

The older six-abstraction map below is retained as a source-navigation aid.
It is not the current component taxonomy for parity governance.
...
| 4 | **State (two-tier)** | `typescript/src/bootstrap/state.ts` (96 fields) + `typescript/src/state/AppStateStore.ts` (~86 fields) | `src/bootstrap/state.py` (33 fields) + `src/state/app_state.py` (5 fields) + `src/utils/store.py` | `_BootstrapState` singleton; `AppState` + `create_app_state_store()` |
...
2. **User-level** — `~/.clawcodex/projects/<project-slug>/memory/MEMORY.md` (loaded by
   `src/memdir/paths.py`).
3. **Team-level** — shared via symlinks. **Currently unported**; track any
   renewed plan under [../plans/active/](../plans/active/).
...
shim does not exist in `src/providers/`. Track renewed API-layer work
under [../plans/active/](../plans/active/) if it becomes a product
priority.
...
should be tracked under [../plans/active/](../plans/active/) if the
audit/production boundary changes again.
