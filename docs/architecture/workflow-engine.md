...
> [`../plans/workflow/workflow-engine-port.md`](../plans/workflow/workflow-engine-port.md) maps every piece onto the Python
...
(`examples/workflows/*.js`) — `meta`, `args` handling, a pure helper, `phase`/`log`, a parallel pair of
...
The original JavaScript form of this workflow is `examples/workflows/02-user-prompt-research.js`; the
Python ports of the demo corpus should live alongside it (e.g. `examples/workflows/*.py`).
