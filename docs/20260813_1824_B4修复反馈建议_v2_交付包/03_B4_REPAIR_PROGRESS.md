...
1. 规则状态：`新增、已生效`。
2. 每项修复、功能新增或性能优化必须执行直接相关的单元测试，并记录测试项目与结果。
3. 多模块改动执行受影响模块的组合测试；针对性测试失败后必须继续修复。
4. 禁止每次小改都运行全量测试。全量测试只用于阶段收尾、跨模块高风险变更、发布、合并前验收或用户明确要求。
5. 后续进度记录必须分别标明：`针对性测试`、`组合测试`、`全量测试`、`外部依赖测试`。
