...
1. `keybindings/__init__.py` — 29 行（类 0，函数 0）

## 3. 关键符号

1. 文件 `keybindings/__init__.py`

## 4. 依赖与调用边（import 级）

1. keybindings/__init__.py → import __future__（外部）
1. keybindings/__init__.py → import json（外部）
1. keybindings/__init__.py → import pathlib（外部）
