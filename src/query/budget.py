...
def resolve_max_cost_usd(explicit: float | None, configured: float | None) -> float:
    """Resolve launch override → setting → unlimited.

    ``0`` keeps the product's "unlimited cost" meaning. A positive explicit
    value wins over settings for the current launch, matching max-turns
    precedence.
    """
