...
class _AgentSession:
    ...
    # The background scheduler service pops due tasks and runs their prompts
    # as internal turns when the session turn gate is free; _build_runtime
    ...
    _background_scheduler: SessionBackgroundScheduler | None = None
    _stop: threading.Event = field(default_factory=threading.Event)
    _lock: threading.Lock = field(default_factory=threading.Lock)
    _turn_gate: threading.RLock = field(default_factory=threading.RLock)
    ...
    def start(self) -> None:
        self._background_scheduler = SessionBackgroundScheduler(
            self._background_scheduler_tick,
            interval_s=0.5,
            name=f"agent-server-scheduler-{self.session_id}",
        )
    ...
    def _run_worker(self) -> None:
        while not self._stop.is_set():
            ...
            if isinstance(item, dict) and item.get("__btw__"):  # side question (/btw)
                self._run_serial_turn(item.get("content"), btw=True)
                self._push_cron_state()
                continue
            if isinstance(item, dict) and item.get("__goal__"):
                ...
                if not goal_live:
                    continue
                outcome = self._run_serial_turn(item.get("content"), internal=True)
            ...
            outcome = self._run_serial_turn(item)
            self._maybe_continue_goal(outcome)
            ...
            # A task that finished while the turn ran, or a scheduled task that
            # came due mid-turn, is handled right after via the same scheduler
            # entry point the background thread uses.
            self._background_scheduler_tick()
    ...
    def _background_scheduler_tick(self) -> None:
        """One background housekeeping tick.

        The dedicated scheduler thread owns periodic checks, while
        ``_turn_gate`` preserves the existing "one model turn mutates the
        session conversation at a time" invariant. If the foreground worker is
        blocked in a user turn (including AskUserQuestion/permission wait), the
        scheduler clock still ticks and publishes any state changes it can, but
        it does not interleave another model turn into the same transcript.
        """
    ...
    def _run_serial_turn(self, prompt, btw: bool = False,
                         internal: bool = False) -> dict | None:
        """Run one turn under the session turn gate."""
        with self._turn_gate:
            return self._run_turn(prompt, btw=btw, internal=internal)
    ...
    def _deliver_task_notifications(self) -> bool:
        ...
        both. Runs under the session turn gate, so it can never interleave with
        a foreground user turn. Returns whether anything was delivered.
        ...
        self._run_serial_turn(build_notification_turn(envelopes), internal=True)
        return True
    ...
    def _fire_due_scheduled(self) -> bool:
        """Background-scheduler consumer: pop every due scheduled task and run
        each prompt as one internal turn. The caller holds the session turn
        gate, so a fire can never interleave with a user turn (CC: "a
        scheduled prompt fires between your turns, not while Claude is
        mid-response"; a task that came due mid-turn fires once when the gate
        is free — no catch-up). Returns whether anything fired."""
        if self.init_error is not None or self._stop.is_set():
            return False
        try:
        ...
        for task in fired:
            ...
            if task.kind == "wakeup":
                ...
                self._run_serial_turn(self._scheduled_turn_content(task), internal=True)
    ...
    async def shutdown(self) -> None:
        ...
        if self._background_scheduler is not None:
            self._background_scheduler.stop(timeout=2.0)
        worker = self._worker
