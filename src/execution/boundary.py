...
class ExecutionBoundary:
    ...
    sandbox_backend: SandboxBackend = field(default_factory=default_sandbox_backend)
    network_policy: NetworkPolicy = field(default_factory=default_network_policy)
    ...
    def check_network(
        ...
        *,
        ...
        return self.network_policy.check_url(url, purpose=purpose)
    ...
    def prepare_sandbox(
        ...
        return self.sandbox_backend.prepare(request, policy)

    def run_sandbox(self, invocation: SandboxInvocation) -> SandboxExecutionResult:
        return self.sandbox_backend.run(invocation)
...
def minimal_execution_boundary(
    *,
    ...
    return ExecutionBoundary(
        env_policy=MinimalEnvPolicy(extra_allowed_keys=tuple(extra_env_keys)),
        network_policy=ConfigurableNetworkPolicy(
            mode=network_mode,
