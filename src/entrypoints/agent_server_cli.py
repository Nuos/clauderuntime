...
def run_agent_server_subcommand(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--max-cost-usd", type=float, default=None, dest="max_cost_usd",
        help="Maximum model spend for this session in USD (0 = unlimited).",
    )
    parser.add_argument(
    ...
    if args.fallback_model and args.fallback_model == args.model:
        print("agent-server: --fallback-model must differ from --model",
              file=sys.stderr)
        return 2
    from src.query.budget import resolve_max_cost_usd, resolve_max_turns
    ...
    agent_config = AgentServerConfig(
        ...
        max_cost_usd=resolve_max_cost_usd(
            args.max_cost_usd,
            float(budget_settings.max_cost_usd or 0.0),
        ),
    )
